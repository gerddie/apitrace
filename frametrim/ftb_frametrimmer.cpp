...
struct FrameTrimmeImpl {
    ...
    PTraceCall Generate(const trace::Call& call, ObjectMap &object_map);
    ...
    ObjectMap m_legacy_programs;
    ObjectMap m_programs;
    ObjectMap m_textures;
    ObjectMap m_fbo;
    ObjectMap m_buffers;
    ObjectMap m_shaders;
    ObjectMap m_renderbuffers;
    ObjectMap m_samplers;
    ObjectMap m_vertex_arrays;
    ObjectMap m_vertex_attrib_pointers;
    ObjectMap m_sync_objects;
...
void FrameTrimmeImpl::start_target_frame()
{
    ...
    for (auto&& va: m_va_is_enabled) {
        ...
        if (buf)
            buf->append_calls(m_required_calls);
    }
